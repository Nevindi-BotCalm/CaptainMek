      ...
      <div className="flex flex-col lg:flex-row items-center justify-center gap-8 xl:gap-12 2xl:gap-16 pb-12">
        ...
        <div className="w-full lg:w-1/2 grid grid-cols-1 sm:grid-cols-2 gap-6 xl:gap-8 2xl:gap-10">
          {steps.map((step) => (
            <div
              key={step.id}
              className={`relative ${step} rounded-2xl p-6 ${step} hover:scale-105 transition-transform duration-300 overflow-hidden shadow-lg`}
            >
              <img
                src={step.image}
                alt={step.title}
                className="absolute inset-0 w-[395px] h-[277px] object-cover"
              />
